...
func main() {
	...
	ingestSrv.RegisterLogStore(logStore)    // 注册日志 API 路由
	go func() {
	...
	// ── 5. 启动仪表板服务（Web UI + WebSocket，含告警状态和日志推送）──────────
	dashSrv, err := dashboard.New(dashboardAddr, db)
	if err != nil {
		fmt.Fprintf(os.Stderr, "仪表板服务启动失败: %v\n", err)
		os.Exit(1)
	}
	dashSrv.SetAlertEngine(alertEng) // 注入告警引擎
	dashSrv.SetLogStore(logStore)    // 注入日志存储，WebSocket 推送包含最近日志
	go func() {
	...
	// 启动日志采集代理（每 10 秒生成一条合成系统事件日志）
	logURL := "http://localhost" + ingestAddr + "/api/v1/logs/single"
	...
	fmt.Println("日志 API: http://localhost:9090/api/v1/logs")
